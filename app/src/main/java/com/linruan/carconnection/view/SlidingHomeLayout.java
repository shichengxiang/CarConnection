...
public class SlidingHomeLayout extends ViewGroup {
    /**
     * 用于完成滚动操作的实例
     */
    ...
    private int rvY = 0;
    private int remainHeight = (int) (getResources().getDisplayMetrics().density * 210);
    private int realHeight = (int) (getResources().getDisplayMetrics().heightPixels - 120 * getResources().getDisplayMetrics().density);
    ...
    public SlidingHomeLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        // 第一步，创建Scroller的实例
    ...
    protected void onFinishInflate() {
        ...
        recyclerView = bottomView.findViewById(R.id.rvHomeMasterList);
        bottomView.setOnTouchListener((v, event) -> {
...
//        promptView.layout(0,getMeasuredHeight(),promptView.getMeasuredWidth(),getMeasuredHeight()+promptView.getMeasuredHeight());
        bottomView.layout(0, getMeasuredHeight() - remainHeight, getMeasuredWidth(), getMeasuredHeight() - remainHeight + realHeight);


    }

    public void expandsMasterLit() {
        if (!isExpand) {
            ObjectAnimator trans = ObjectAnimator.ofFloat(bottomView, "translationY", 0f, -(realHeight - remainHeight));
            trans.setDuration(500);
            trans.start();
            isExpand = true;
            mOnChangeListener.onChange(isExpand);
        }
    }

    public void shrinkMasterList() {
        if (isExpand) {
            ObjectAnimator trans = ObjectAnimator.ofFloat(bottomView, "translationY", -realHeight + remainHeight, 0f);
            trans.setDuration(500);
            trans.start();
            isExpand = false;
            mOnChangeListener.onChange(isExpand);
